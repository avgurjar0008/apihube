...
const origins = [
    ...
    "http://127.0.0.1:5174",
    "http://127.0.0.1:5175",
    "https://apihube.vercel.app", process.env.CLIENT_URL
  ].filter(Boolean);
app.use(cors({
  origin(origin, callback) { if (!origin || origins.includes(origin)) return callback(null, true); callback(new Error("Origin not allowed by CORS.")); },
  credentials: true,
